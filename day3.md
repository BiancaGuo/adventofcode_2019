...
for step in s1_l:
    if step[0]=="R":
        for s1_x in range(0,int(step[1:])):
            s1_pixel.append((x+s1_x,y))
        x+=int(step[1:])
    elif step[0]=="L":
        for s1_x in range(1,int(step[1:])+1):
            s1_pixel.append((x-s1_x,y))
        x-=int(step[1:])
    elif step[0]=="U":
        for s1_y in range(1,int(step[1:])+1):
            s1_pixel.append((x,y+s1_y))
        y+=int(step[1:])
    else:
        for s1_y in range(1,int(step[1:])+1):
            s1_pixel.append((x,y-s1_y))
...
for step in s2_l:
    if step[0]=="R":
        for s2_x in range(1,int(step[1:])+1):
            s2_pixel.append((x+s2_x,y))
        x+=int(step[1:])
    elif step[0]=="L":
        for s2_x in range(1,int(step[1:])+1):
            s2_pixel.append((x-s2_x,y))
        ...
        for s2_y in range(1,int(step[1:])+1):
            s2_pixel.append((x,y+s2_y))
        y+=int(step[1:])
    else:
        for s2_y in range(1,int(step[1:])+1):
            s2_pixel.append((x,y-s2_y))
...
intersection = list(set(s1_pixel).intersection(set(s2_pixel)))
min= abs(intersection[0][0]-0)+abs(intersection[0][1]-0)
for pix in intersection:
    if (abs(pix[0]-0)+abs(pix[1]-0))<min:
        min=abs(pix[0]-0)+abs(pix[1]-0)
print(min)
```
...
intersection = list(set(s1_pixel).intersection(set(s2_pixel)))
min= dic_s1[(intersection[0][0],intersection[0][1])]+dic_s2[(intersection[0][0],intersection[0][1])]
for pix in intersection:
    if (dic_s1[(pix[0],pix[1])]+dic_s2[(pix[0],pix[1])])<min:
        min=dic_s1[(pix[0],pix[1])]+dic_s2[(pix[0],pix[1])]
